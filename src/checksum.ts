...
const crc16: TinyFrameChecksum = {
  ...
  sum(buffer: Iterable<number>) {
    ...
    for (const byte of buffer) {
      sum = (sum >>> 8) ^ CRC16_TABLE[(sum ^ byte) & 0xFF]
    }
...
const crc32: TinyFrameChecksum = {
  ...
  sum(buffer: Iterable<number>) {
    ...
    for (const byte of buffer) {
      sum = (sum >>> 8) ^ CRC32_TABLE[(sum ^ byte) & 0xFF]
    }
...
export { crc8, crc16, crc32, xor }

export default { xor, crc8, crc16, crc32 }
